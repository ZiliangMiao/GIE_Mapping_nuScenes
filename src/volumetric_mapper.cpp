...
VOLMAPNODE::VOLMAPNODE()
{
    ...
    // {
    //     s_odom_sub.subscribe(_nh,"/odom", 1);
    //     s_pntcld_sub.subscribe(_nh,"lidar_top", 1); // for nuscenes only
    //     pntcld_sync = new message_filters::Synchronizer<pntcld_sync_policy> (pntcld_sync_policy(30), s_pntcld_sub, s_odom_sub);
    //     pntcld_sync->setMaxIntervalDuration(ros::Duration(0.05));
    //     pntcld_sync->registerCallback(boost::bind(&VOLMAPNODE::CB_pntcld_odom,this,_1,_2));
    // }
    ...
    // }
    // if (param.display_loc_edt || param.save_loc_edt)  // question mark [miaozl]
    // {
    //     _edt_rviz_pub = _nh.advertise<PntCldI>("edt_map", 1);
    //     _edt_pnt_cld = PntCldI::Ptr(new PntCldI);
    //     _edt_pnt_cld->header.frame_id = "map";
    // }
    // if (param.display_glb_edt || param.display_glb_edt)  // question mark [miaozl]
    // {
    ...
    // // Timer
    // _mapTimer = _nh.createTimer(ros::Duration(0.05), &VOLMAPNODE::publishMap, this);
    ...
    // rosbag::MessageInstance, for nuscenes only
    rosbag::Bag bag;
    string bag_name;

    // read ros bag
    _nh.getParam("GIE_mapping/bag_name",bag_name);
    std::cout <<  " -- [Test] Bag name:" << bag_name << std::endl;
    bag.open(bag_name, rosbag::bagmode::Read);
    // topics: lidar point cloud & odometry
    vector<string> topics;
    string odom_topic, lidar_topic;
    _nh.getParam("GIE_mapping/odom_topic", odom_topic);
    _nh.getParam("GIE_mapping/cloud_topic", lidar_topic);
    ...
    FILE *fp = fopen(param.log_dir.c_str(),"w");
    ...
    for (rosbag::MessageInstance const m: view) {
        ...
        if (m.getTopic() == odom_topic) {
            ...
            cloud_time = m.getTime();
        }
        else{
            std::cout << " -- [Test] Read outlier topic: " << m.getTopic() << std::endl;
        }
        ...
        if (cloud_msg != nullptr && odom_msg != nullptr && odom_time == cloud_time) {
            ...
            VOLMAPNODE::CB_pntcld_odom (cloud_msg, odom_msg);
            ...
            std::cout << "----- Update Hash OGM -----" << std::endl;
            update_ext_map();
            _hash_map->updateHashOGM(_msg_mgr.got_pnt_cld  && param.data_case != "laser3D" && param.data_case != "nusc",
                                    _time, param.display_glb_ogm && !param.display_glb_edt, ext_obs);
            ...
            std::cout << "----- Update Batch EDT -----" << std::endl;
            ...
            //do visualization
             std::cout << "----- Visualize and Save -----" << std::endl;
            auto vis_start = std::chrono::steady_clock::now();
            if (scan_idx % param.vis_interval == 0)
            {   
...
tf::Transform VOLMAPNODE::odom2trans()  {
    ...
    // std::cout << "Nusc odometry translation: " << _odom_ptr->pose.pose.position.x << " " << _odom_ptr->pose.pose.position.y << " " << _odom_ptr->pose.pose.position.z << std::endl;
    ...
    }else if (param.data_case == "nusc")
    {
        ...
        tf::transformTFToEigen( trans, transEigen);
        Eigen::Matrix4d transMat = transEigen.matrix();
        transMat = transMat*param.T_L_V;  // from lidar to vehicle; from vehicle to world
        transEigen.matrix() = transMat;
        tf::Transform transCorrect;
        tf::transformEigenToTF(transEigen, transCorrect);
        return  transCorrect;
...
void VOLMAPNODE::CB_pntcld(const sensor_msgs::PointCloud2::ConstPtr& msg)
{
    ...
    {   
        ...
        int num_rings = 32;
        int num_ring_pts = pcl_cloud->points.size() / num_rings;
        ...
        // printf("Ring angle map: %f %f %f %f\n", ring_angle_map[0]/M_PI*180.0, ring_angle_map[1]/M_PI*180.0, ring_angle_map[2]/M_PI*180.0, ring_angle_map[3]/M_PI*180.0);
        // printf("Ring angle map: %f %f %f %f\n", ring_angle_map[4]/M_PI*180.0, ring_angle_map[5]/M_PI*180.0, ring_angle_map[6]/M_PI*180.0, ring_angle_map[7]/M_PI*180.0);
        // printf("Ring angle map: %f %f %f %f\n", ring_angle_map[8]/M_PI*180.0, ring_angle_map[9]/M_PI*180.0, ring_angle_map[10]/M_PI*180.0, ring_angle_map[11]/M_PI*180.0);
        // printf("Ring angle map: %f %f %f %f\n", ring_angle_map[12]/M_PI*180.0, ring_angle_map[13]/M_PI*180.0, ring_angle_map[14]/M_PI*180.0, ring_angle_map[15]/M_PI*180.0);
        // printf("Ring angle map: %f %f %f %f\n", ring_angle_map[16]/M_PI*180.0, ring_angle_map[17]/M_PI*180.0, ring_angle_map[18]/M_PI*180.0, ring_angle_map[19]/M_PI*180.0);
        // printf("Ring angle map: %f %f %f %f\n", ring_angle_map[20]/M_PI*180.0, ring_angle_map[21]/M_PI*180.0, ring_angle_map[22]/M_PI*180.0, ring_angle_map[23]/M_PI*180.0);
        // printf("Ring angle map: %f %f %f %f\n", ring_angle_map[24]/M_PI*180.0, ring_angle_map[25]/M_PI*180.0, ring_angle_map[26]/M_PI*180.0, ring_angle_map[27]/M_PI*180.0);
        // printf("Ring angle map: %f %f %f %f\n", ring_angle_map[28]/M_PI*180.0, ring_angle_map[29]/M_PI*180.0, ring_angle_map[30]/M_PI*180.0, ring_angle_map[31]/M_PI*180.0);
        // printf("Ring 31 max: %f\n", ring_31_max/M_PI*180.0);
        ...
        if(!_vlp_map_maker.is_initialized())
        {
            ...
            MulScanParam mp_nusc(num_ring_pts, num_rings, 70.0f,  azimuth_inc,  -M_PI,
                                 ...
                                 ring_angle_map[4], ring_angle_map[5], ring_angle_map[6], ring_angle_map[7],
                                 ring_angle_map[8], ring_angle_map[9], ring_angle_map[10], ring_angle_map[11],
                                 ring_angle_map[12], ring_angle_map[13], ring_angle_map[14], ring_angle_map[15],
                                 ring_angle_map[16], ring_angle_map[17], ring_angle_map[18], ring_angle_map[19],
                                 ring_angle_map[20], ring_angle_map[21], ring_angle_map[22], ring_angle_map[23],
                                 ring_angle_map[24], ring_angle_map[25], ring_angle_map[26], ring_angle_map[27],
                                 ring_angle_map[28], ring_angle_map[29], ring_angle_map[30], ring_angle_map[31], ring_31_max);
    ...
    *_pntcld_ptr = *msg;  // boost::shared_ptr<sensor_msgs::PointCloud2>
